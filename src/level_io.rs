...
pub fn load_button_interaction(
    ...
    existing_dialog: Query<Entity, Or<(With<SaveDialog>, With<LoadDialog>, With<OverwriteDialog>, With<GenDialog>)>>,
    font: Res<GameFont>,
) {
...
fn level_summary(path: &std::path::Path) -> String {
    let Ok(json) = fs::read_to_string(path) else { return String::new() };
    let Ok(level) = serde_json::from_str::<LevelData>(&json) else { return String::new() };
    let s = level.board_size;
    let mut srcs = 0u32; let mut goals = 0u32; let mut mechs = 0u32; let mut inv = 0u32;
    for &(_, _, ref k, sol) in &level.tiles {
        if sol { inv += 1; }
        match k {
            TileKind::Source(..) => srcs += 1, TileKind::Goal(..) => goals += 1,
            TileKind::Floor | TileKind::Empty => {}
            _ => mechs += 1,
        }
    }
    let diff = level.difficulty.map(|d| format!(" | D:{d}")).unwrap_or_default();
    format!("{s}x{s} | {srcs}src {goals}goal | {mechs} tiles | {inv} inv{diff}")
}

fn spawn_load_dialog(commands: &mut Commands, f: &Handle<Font>) {
    let tf = gf(DIALOG_BODY_FONT, f);
    let tc = TextColor(Color::WHITE);
    let hc = TextColor(rgb(DIALOG_EMPTY_TEXT));
    let hf = gf(COUNT_FONT, f);
    let dir = levels_dir();
    ...
    if let Ok(readdir) = fs::read_dir(&dir) {
        for entry in readdir.flatten() {
            ...
            if path.extension().is_some_and(|e| e == "json") { if let Some(stem) = path.file_stem() {
                ...
                let summary = level_summary(&path);
    ...
    spawn_dialog(commands, LoadDialog, pn, |panel| {
        ...
        panel.spawn(Node { flex_direction: FlexDirection::Row, width: Val::Px(DIALOG_LIST_WIDTH),
            ...
            row.spawn((Node { flex_direction: FlexDirection::Column, row_gap: Val::Px(DIALOG_LIST_GAP),
                ...
                if entries.is_empty() {
                    list.spawn((Text::new("No saved levels"), tf.clone(), hc));
                }
                    ...
                    .with_children(|row| {
                        row.spawn((Button, LoadDialogEntry(name.clone()),
                            Node { padding: UiRect::axes(Val::Px(LOAD_DIALOG_ROW_GAP), Val::Px(DIALOG_BTN_PAD.1)),
                                ...
                                row_gap: Val::Px(2.0), ..default() },
                            BackgroundColor(btn_bg()),
                        )).with_children(|btn| {
                            btn.spawn((Text::new(name.clone()), tf.clone(), tc));
                            if !summary.is_empty() {
                                btn.spawn((Text::new(summary.clone()), hf.clone(), hc));
                            }
                        });
...
pub fn load_dialog_buttons(
    ...
    loaded_name.name = Some(level.name.clone());
    loaded_name.gen_seed = level.seed;
    loaded_name.gen_difficulty = level.difficulty;

    for entity in &tiles { commands.entity(entity).despawn(); }
    board_size.0 = level.board_size.clamp(MIN_BOARD_SIZE, MAX_BOARD_SIZE);
    populate_board(&mut commands, board_size.0, &level.tiles, &assets);
    validated.0 = false;
